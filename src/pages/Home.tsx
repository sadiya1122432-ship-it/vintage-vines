      ...
      <section className="container mx-auto px-6 space-y-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
          {products.length > 0 ? products.map((product, idx) => (
            ...
            >
              <div className="aspect-[3/4] overflow-hidden bg-white relative premium-shadow rounded-2xl flex items-center justify-center p-4">
                <motion.img
                  whileTap={{ scale: 0.95 }}
                  src={product.imageUrl}
                  alt={product.title}
                  className="max-w-full max-h-full object-contain transition-transform duration-1000 group-hover:scale-110 cursor-pointer"
                  referrerPolicy="no-referrer"
              ...
              <div className="space-y-4 text-center">
                <h3 
                  onClick={() => navigate(`/product/${product.id}`)}
                  className="font-serif text-3xl font-medium text-ink cursor-pointer hover:text-gold transition-colors"
                >
                  {product.title}
                </h3>
