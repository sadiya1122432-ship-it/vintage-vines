...
app.post("/api/products", async (req, res) => {
  ...
  const result = db_sqlite.prepare("INSERT INTO products (title, description, price, imageUrl) VALUES (?, ?, ?, ?)").run(title, description, price, imageUrl);
  const productId = result.lastInsertRowid;

  // Google Sheet-এও অ্যাড করা
  try {
    await appendToSheet('Products', [[productId.toString(), title, description, price.toString(), imageUrl]]);
  } catch (err) {
...
app.put("/api/products/:id", (req, res) => {
  const { id } = req.params;
  const { title, description, price, imageUrl } = req.body;
  db_sqlite.prepare("UPDATE products SET title = ?, description = ?, price = ?, imageUrl = ? WHERE id = ?").run(title, description, price, imageUrl, id);
  res.json({ success: true });
});
  ...
  try {
    ...
    db_sqlite.transaction(() => {
      ...
      for (const row of productsToSync) {
        ...
        if (title) {
          insertProduct.run(title, description, price, imageUrl);
        }
      }
    })();
